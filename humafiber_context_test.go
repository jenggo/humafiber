...
func FiberMiddlewareUserValue(c fiber.Ctx) error {
	value := c.Get(HeaderNameFiberUserValue)
	if value != "" {
		// Fiber v3 stores user values directly on Ctx, not on Context()
		c.Locals(string(contextValueFiberUserValue), value)
	}
	return c.Next()
}

func FiberMiddlewareUserContext(c fiber.Ctx) error {
	value := c.Get(HeaderNameFiberUserContext)
	if value != "" {
		// For fiber v3, we need to use context.WithValue on the context
		// and set it back using c.SetUserContext(ctx)
		original := c.Context()
		result := context.WithValue(original, contextValueFiberUserContext, value)
		c.SetContext(result) // Use SetContext instead of SetUserContext
